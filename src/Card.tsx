...
function AbsoluteFill(props: HTMLProps<HTMLDivElement>) {
  return (
    <div
      style={{
        position: 'absolute',
...
function Line({ color }: { color: string }) {
  return (
    ...
    >
      <div className="h-[2px] line-gradient rotate-180 w-[171px]" />
      <div className="h-[2px] line-gradient w-[171px]" />
    </div>
  )
}
...
function HLine({
  ...
  return (
    ...
    >
      <div className="w-[2px] line-gradient h-[111px] rotate-180" />
      <div className="w-[2px] line-gradient h-[111px]" />
    </div>
  )
}
...
function DynamicImage({
  ...
  if (lucky && stage !== 3) {
    const notFitStyles = notFitLuckies.includes(id)
      ...
      : {}
    const luckyStyles = luckyWidthStyles[id] || {}
    ...
    return (
      <Img
        ...
        style={{
          marginBottom: calcMarginBottom({ id, stage, lucky }) + 'px',
          ...notFitStyles,
          ...luckyStyles,
        }}
      />
  ...
  return (
    <AbsoluteFill className="bg-brand-dark absolute top-0 left-0 w-full h-full">
      <AbsoluteFill className="m-auto relative flex flex-col">
        ...
        <div
          style={
            {
              filter: lucky
                ? 'drop-shadow(4px 4px var(--blur-size, 4px) var(--beast-color, #06c976))'
                : 'none',
        ...
        <div className="absolute bottom-0 py-4 w-full">
          <h1 className="text-center text-white font-priory-san uppercase font-semibold text-[28px] -mb-[11px] leading-none">
            {specieStage.name} - {specieStage.formatId}
          </h1>
      ...
      {[...Array(32)].map((_, index) => (
        <div
          className="w-full h-[1px] absolute bg-cyan-100 opacity-25"
          style={{
            top: `${285 - index * 5}px`,
          }}
          key={index}
